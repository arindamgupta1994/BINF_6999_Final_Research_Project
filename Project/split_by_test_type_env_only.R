...
names(test_split_list) <- sapply(test_split_list, function(df) {
  paste(df$TestGroup[1])
})

# Check the names
names(test_split_list)

# Confirm total rows add up
total_rows_split <- sum(sapply(test_split_list, nrow))
total_rows_original <- nrow(merged_df)
...
dir.create("../Project_Trial_2 - split_by_test_type_env_only/H_Heatmaps", showWarnings = FALSE)
...
ggsave("../Project_Trial_2 - split_by_test_type_env_only/H_Heatmaps/Combined_H_Heatmaps.png",
       combined_heatmap,
...
dir.create("../Project_Trial_2 - split_by_test_type_env_only/LM_Plots", showWarnings = FALSE)
...
# Save to file
ggsave("../Project_Trial_2 - split_by_test_type_env_only/LM_Plots/Combined_Pred_vs_Actual_with_labels.png",
       plot = final_plot,
       width = 15, height = 10, dpi = 300)

####################### 6. LM WITHOUT NMF ##########################

dir.create("../Project_Trial_2 - split_by_test_type_env_only/LM_Plots_NoNMF", showWarnings = FALSE)
...
# Save to file
ggsave("../Project_Trial_2 - split_by_test_type_env_only/LM_Plots_NoNMF/Combined_Pred_vs_Actual_NoNMF_with_labels.png",
       plot = final_plot,
       width = 15, height = 10, dpi = 300)
